     ...
     const std::vector<unsigned int> &subdivisions)
{
  ...
  // create triangulation
  {
    ...
    if (do_simplex)
      {
        ...
        GridGenerator::subdivided_hyper_rectangle(tria_temp,
                                                  subdivisions,
                                                  p1,
                                                  p2);
    ...
    GridTools::partition_triangulation(
      Utilities::MPI::n_mpi_processes(MPI_COMM_WORLD), tria_serial);

    tria.create_triangulation(
      TriangulationDescription::Utilities::
        create_description_from_triangulation(tria_serial, MPI_COMM_WORLD));
  }
  ...
  if (do_simplex)
    {
      mapping = std::make_unique<MappingFE<dim, spacedim>>(
  ...
  PoissonOperator poisson_operator(
    *mapping, dof_handler, constraints, *quadrature, x, b);

  {
    const auto start = std::chrono::system_clock::now();

    const unsigned int n_rep = 100;
    ...
    if (Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0)
      std::cout << tria.n_global_active_cells() << " " << dof_handler.n_dofs()
                << " " << duration / n_rep << " "
                << dof_handler.n_dofs() / duration / 1e6 * n_rep << std::endl;
  }
}
...
main(int argc, char **argv)
{
  ...
  const bool         do_matrix_free = argc <= 4 ? 1 : atoi(argv[4]);
  const unsigned int s_max          = argc <= 5 ? 9 : atoi(argv[5]);
